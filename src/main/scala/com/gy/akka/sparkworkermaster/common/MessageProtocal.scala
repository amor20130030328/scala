package com.gy.akka.sparkworkermaster.common
...
/**
  ...
  *
  */
//worker注册信息 // MessageProtocal.scala
case class RegisterWorkerInfo( id : String , cpu : Int , ram : Int)

//这个是workerInfo，这个信息将来是保存到 master 的 hm（该hashmap 是用于管理worker）
// 将来这个workerInfo 会扩展（比如增加worker上一次的心跳时间）
...
//worker 每隔一定时间由定时器触发，而向master 发现的协议消息
case object SendHeartBeat
//worker每隔一定时间由定时器触发,而向master 发现的协议消息
case class HeartBeat(id : String)

//master 给自己发送一个触发检查超时worker 的信息
case object  StartTimeOutWorker
//master 给自己发消息，检测worker，对于心跳超时的
